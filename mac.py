...
def automateXcodeActions(project_path, workspace_path, scheme_name, udid_device):
    ...
    try:
        if arrangement_type not in ["left", "right"]:
            print("Invalid arrangement type. Use 'left' or 'right'.")
            return

        applescript = f'''
            tell application "System Events"
                key code 0 using {{command down, option down}}  -- Press Command + Option + {arrangement_type[0].capitalize()}
            end tell
        '''
        subprocess.run(["osascript", "-e", applescript])
        print(f"All windows arranged to {arrangement_type} successfully.")
...
# Example: Running a keyboard shortcut (Command + N) to open a new window in Finder
